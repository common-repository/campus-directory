...
//  ____________MAIN METHODS____________
	...
	function admin_msg( $msg = '', $class = "updated" ) {
		if ( empty( $msg ) )
			$msg = __( 'Settings <strong>saved</strong>.', 'campus-directory' );
	...
	function submit_button( $value = '', $action = 'action', $class = "button" ) {
		if ( is_array( $value ) ) {
			extract( wp_parse_args( $value, array(
				'value' => __( 'Save Changes', 'campus-directory' ),
				'action' => 'action',
				'class' => 'button',
				'ajax' => true
			) ) );
...
//  ____________INTERNAL METHODS____________
	...
	private function check_args() {
		...
		$this->args = wp_parse_args( $this->args, array(
			...
			'page_slug' => '',
			'nonce' => '',
			'action_link' => __( 'Settings', 'campus-directory' ),
			'ajax_submit' => false,
...
jQuery( document ).ready( function( $ ){
	...
	$( ':submit' ).click( function( ev ){
		...
		data.push( {name: '_ajax_submit', value: '<?php echo esc_attr($this->pagehook); ?>'} );
...
<?php
	...
	function _action_link( $links ) {
		$url = esc_url(add_query_arg( 'page', $this->args['page_slug'], admin_url( $this->args['parent'] ) ));
