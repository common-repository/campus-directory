...
class P2P_Widget extends scbWidget {
	...
	function __construct() {
		parent::__construct( 'p2p', __( 'Posts 2 Posts', 'campus-directory' ), array(
			'description' => __( 'A list of posts connected to the current post', 'campus-directory' )
		) );
	}
	...
	function form( $instance ) {
		...
		echo html( 'p', $this->input( array(
			'type' => 'text',
			'name' => 'title',
			'desc' => __( 'Title:', 'campus-directory' )
		), $instance ) );
		...
		echo html( 'p', $this->input( array(
			...
			'name' => 'ctype',
			'values' => $ctypes,
			'desc' => __( 'Connection type:', 'campus-directory' ),
			'extra' => "style='width: 100%'"
		), $instance ) );

		echo html( 'p',
			__( 'Connection listing:', 'campus-directory' ),
			'<br>',
			$this->input( array(
				'type' => 'radio',
				'name' => 'listing',
				'values' => array(
					'connected' => __( 'connected', 'campus-directory' ),
					'related' => __( 'related', 'campus-directory' )
				),
			), $instance )
		);
	}
