...
class Emd_Person extends Emd_Entity {
	...
	protected $app = 'campus_directory';
	...
	public function column_headers($columns) {
		...
		if (!empty($ent_list[$this->post_type]['featured_img'])) {
			$columns['featured_img'] = __('Featured Image', 'campus-directory');
		}
	...
	public function change_content($content) {
		...
		if (get_post_type() == $this->post_type && is_single()) {
			...
			emd_get_template_part($this->app, 'single', 'emd-person');
			do_action('emd_single_after_content', $this->app, $this->post_type);
			$layout = ob_get_clean();
		} elseif (is_post_type_archive('emd_person')) {
			ob_start();
			do_action('emd_single_before_content', $this->app, $this->post_type);
			emd_get_template_part($this->app, 'archive', 'emd-person');
			do_action('emd_single_after_content', $this->app, $this->post_type);
			$layout = ob_get_clean();
		} elseif (is_tax('person_area') && $post->post_type == $this->post_type) {
			ob_start();
			do_action('emd_single_before_content', $this->app, $this->post_type);
			emd_get_template_part($this->app, 'taxonomy', 'person-area-emd-person');
			do_action('emd_single_after_content', $this->app, $this->post_type);
			$layout = ob_get_clean();
		} elseif (is_tax('person_title') && $post->post_type == $this->post_type) {
			ob_start();
			do_action('emd_single_before_content', $this->app, $this->post_type);
			emd_get_template_part($this->app, 'taxonomy', 'person-title-emd-person');
			do_action('emd_single_after_content', $this->app, $this->post_type);
			$layout = ob_get_clean();
		} elseif (is_tax('person_rareas') && $post->post_type == $this->post_type) {
			ob_start();
			do_action('emd_single_before_content', $this->app, $this->post_type);
			emd_get_template_part($this->app, 'taxonomy', 'person-rareas-emd-person');
			do_action('emd_single_after_content', $this->app, $this->post_type);
			$layout = ob_get_clean();
		} elseif (is_tax('person_location') && $post->post_type == $this->post_type) {
			ob_start();
			do_action('emd_single_before_content', $this->app, $this->post_type);
			emd_get_template_part($this->app, 'taxonomy', 'person-location-emd-person');
			do_action('emd_single_after_content', $this->app, $this->post_type);
			$layout = ob_get_clean();
		} elseif (is_tax('directory_tag') && $post->post_type == $this->post_type) {
			ob_start();
			do_action('emd_single_before_content', $this->app, $this->post_type);
			emd_get_template_part($this->app, 'taxonomy', 'directory-tag-emd-person');
			do_action('emd_single_after_content', $this->app, $this->post_type);
			$layout = ob_get_clean();
		}
		if ($layout != "") {
